      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div ref={ref} className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {FEATURES.map((f, index) => (
            <div
              key={f.title}
              className={`flex items-center gap-4 p-4 rounded-2xl hover:bg-amber-50 dark:hover:bg-white/5 transition-all group reveal-scale stagger-${index + 1} ${isVisible ? 'revealed' : ''}`}
            >
