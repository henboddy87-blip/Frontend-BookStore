      ...
      <div className="max-w-3xl mx-auto px-4 py-16">
        {FAQS.map((category, catIndex) => (
          <div key={category.category} className="mb-10">
            <h2 className="text-2xl font-black text-amber-900 dark:text-amber-400 mb-6 font-serif">
              {category.category}
            </h2>
            <div className="space-y-4">
              {category.questions.map((faq, qIndex) => {
                ...
                return (
                  <div
                    key={qIndex}
                    className="bg-white dark:bg-dark-card border border-amber-100 dark:border-white/5 rounded-2xl overflow-hidden shadow-sm transition-all"
                  >
                    <button
                      onClick={() => toggle(catIndex, qIndex)}
                      className="w-full text-left px-6 py-4 flex items-center justify-between font-bold text-amber-950 dark:text-gray-200 hover:bg-amber-50 dark:hover:bg-white/5 transition-colors"
                    >
                      {faq.q}
                      {isOpen ? (
                    ...
                    {isOpen && (
                      <div className="px-6 pb-5 pt-2 text-amber-900/70 dark:text-gray-400 text-sm leading-relaxed">
                        {faq.a}
                      </div>
                    )}
                  </div>
